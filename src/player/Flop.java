...
public class Flop {
    ...
    static double badEquity = Constants.flopAverage;
    ...
    public static String takeAction(ProcessActions action, double equity, int potSize, int turn, boolean isButton, Historian mister, int betType){
        ...
        if (Brain.handsIn < 150){ //not enough info yet, play default
            if (turn == 1){
            ...
            double foldFreq = mister.flopFoldingFrequencies();
            ...
            if (!isButton && turn == 1){ //we act first here
                ...
                double wentToFlopFreq = mister.wentToFlopFrequency();
                avgPercentileOfOpponentPlayedHands = 1 - ((1 - avgPercentileOfOpponentPlayedHands) * wentToFlopFreq);
